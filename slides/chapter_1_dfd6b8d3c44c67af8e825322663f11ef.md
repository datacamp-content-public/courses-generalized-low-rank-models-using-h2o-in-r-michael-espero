...
`@script`
Now let's load the "DataExplorer" package to access a convenient function that builds a horizontal lollipop chart showing proportions of missing values in these variables. We can use the pipe operator to first, convert an H2OFrame into a tibble data frame- and then, pass the tibble of imperfect data with missing values to the plot_missing function. 
...
`@script`
Here's the G-L-R-M you trained in the last exercise with rank 64 and seed 143 indicated for reproducibility. It's available in the environment as a H2O-Dim_Reduction-Model.
