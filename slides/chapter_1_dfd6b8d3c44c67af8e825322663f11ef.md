...
`@script`
Here's the G-L-R-M you trained in the last exercise with rank 64. It's available in the environment as a H2O-Dim_Reduction-Model.
...
`@citations`
Now we're going to use h2o's reconstruct function, applying the GLRM designed for imputation to predict missing values. Here we pass our previously trained generalized low rank model as the first argument and our original imperfect training data as the second. The result should be a new version of the original, imperfect training data- this time free from missing values.
